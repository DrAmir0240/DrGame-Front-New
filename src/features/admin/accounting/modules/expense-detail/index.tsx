...
import { useExpenseDetail } from "../../apis";
...
export default function ExpenseDetailPage({ id }: Props) {
  ...
  if (!expense) {
    return <div className="text-center py-20 text-muted-foreground">هزینه یافت نشد</div>;
  }

  return (
    <div className="space-y-6">
      <PageHeader title={`هزینه #${expense.id}`} description={expense.account_side?.name}>
          ...
          <div className="bg-neutral-0 border border-neutral-200 rounded-xl p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <p className="font-medium mt-1">{expense.account_side?.name || "—"}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <div className="mt-1"><StatusBadge status={expense.payment_status} /></div>
              </div>
              <div>
                <span className="text-muted-foreground">تاریخ ایجاد:</span>
                <p className="font-medium mt-1">{new Date(expense.created_at).toLocaleDateString("fa-IR")}</p>
              </div>
            </div>
          ...
          <div className="bg-neutral-0 border border-neutral-200 rounded-xl p-6">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-bold text-lg">{formatPrice(expense.amount)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">تخفیف:</span>
                <span className="font-medium">{formatPrice(expense.discount)}</span>
              </div>
              <div className="flex justify-between border-t pt-3">
                <span className="text-muted-foreground">پرداخت شده:</span>
                <span className="font-medium text-emerald-600">{formatPrice(expense.paid_amount)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">مانده:</span>
                <span className={`font-bold ${expense.remaining_amount > 0 ? "text-red-600" : "text-emerald-600"}`}>
                  {formatPrice(expense.remaining_amount)}
                </span>
              </div>
            </div>
          </div>
