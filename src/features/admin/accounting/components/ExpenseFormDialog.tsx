...
import { useCreateExpense, useUpdateExpense, useAccountSidesDropdown, useCategoryDropdown } from "../apis";
...
export default function ExpenseFormDialog({ open, onClose, editItem }: Props) {
  ...
  async function onFormSubmit(data: ExpenseFormData) {
    try {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()} title={editItem ? "ویرایش هزینه" : "هزینه جدید"} className="max-w-lg"
      footer={<>
        <Button type="button" variant="outline" onClick={onClose}>انصراف</Button>
        <Button type="submit" form="expense-form" disabled={isPending}>{isPending ? "در حال ذخیره..." : editItem ? "ذخیره" : "ایجاد"}</Button>
      </>}>
